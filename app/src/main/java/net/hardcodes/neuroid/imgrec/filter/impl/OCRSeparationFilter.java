...
//        for (int i = 0; i < height; i++) {
    ...
    private void BFStraverseAndSave(int startI, int startJ) {
        ...
        ImageAndroid letter = new ImageAndroid(letterWidth, letterHeight, originalImage.getType());
    ...
    private void saveToFile(ImageAndroid img, String letterName) {
        ImageAndroid crop = img;
        if (cropHeight != 0 || cropWidth != 0) {
            OCRCropImage ci = new OCRCropImage();
            ci.setDimension(cropWidth, cropHeight);
            crop = ci.processImage(img);
        }
        ImageUtilities.save(crop, location + letterName + ".png");
    }
